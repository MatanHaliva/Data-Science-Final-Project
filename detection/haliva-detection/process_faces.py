...
class ProcessFaces(Process):
    ...
    def cluster_faces_from_video(self):
        ...
        while True:
            ...
            temp = self.face_detection.detect_faces(frame, str(self.context_id),
                                                    ConfigService.face_detection_threshold())
            faces.extend(temp)
        ...
        while True:
            ...
            for detection in detections:
                ...
                if name_of_class_id == "person":
                    ...
                    self.detect_extra_info_about_person(image, detection, face_encodings, pca_df_all_data,
                                                        detection_time)
                else:
                    message: DetectionDto = DetectionDto(self.context_id, detection_time, name_of_class_id,
                                                         "{} was detected".format(name_of_class_id),
    ...
    def detect_extra_info_about_person(self, frame, detection, face_encodings, pca_df_all_data, detection_time):
        ...
        for face in faces:
            ...
            message: FaceDetectionDto = FaceDetectionDto(self.context_id, detection_time, DetectionType.Face.value,
                                                         "face was detected", detection["confidence"],
                                                         face["face_location"], face_id["cluster"],
                                                         face["face_encoding"])
            DetectionApiConnector.create_detection(message)

            if with_Mask:
                if (mask >= ConfigService.mask_detection_threshold()):
                    message: MaskDetectionDto = MaskDetectionDto(self.context_id, detection_time,
                                                                 DetectionType.Mask.value, "mask was detected",
                                                                 np.float64(mask), True)
                    DetectionApiConnector.create_detection(message)
